...
void test_I2C1_scan(void) // This didn't work for touch scan Comment this while checking for tuoch scan
{
	int i = 0;
	static int k = 0;
	static int j = 0;
	...
	for (i=0;i<0x7F;i++)
	{
		...
		dbg_str_hex8("Device NOT Detected at:",i);
		vTaskDelay(pdMS_TO_TICKS(1000));
	}
}
...
//    HAL_I2C0_Select();
    ...
    errcode = HAL_I2C_Read(0x38, 0x00, &i2c_test_data, 1);
    dbg_str_hex8("test:",i2c_test_data);

    errcode = HAL_I2C_Read(0x38, 0xA8, &i2c_test_data, 1);
    dbg_str_hex8("test:",i2c_test_data);

//    test_I2C0_scan();
//    test_I2C1_scan(); // This didn't work for touch scan Comment this while checking for tuoch scan

    dbg_str_int("touchpad_init:",touchpad_init());


    while (1)
    {
    	dbg_str_int("x_data", read_x_data());
    	dbg_str_int("y_data", read_y_data());
    }
    vTaskDelete( NULL );
}
